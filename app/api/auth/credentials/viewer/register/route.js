...
export async function POST(req) {
  ...
  try {
    ...
    const user = await prisma.user.create({
      data: { email, username, name, password: hashedPassword, role: "VIEWER", NotificationSetting: { create: {} } },
      include: { NotificationSetting: true, }
    });

    // send welcome mail
    const subject = "Welcome to LMS!";
    const dashboardLink = process.env.NEXTAUTH_URL + '/viewer/dashboard';
    const helpLink = process.env.NEXTAUTH_URL + '/viewer/dashboard';
